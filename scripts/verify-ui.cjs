...
(async()=>{
 ...
 await page.locator('.current-text').filter({hasText:'Otro libro.'}).waitFor();
 await page.getByRole('button',{name:'Biblioteca',exact:true}).click();
 await page.getByRole('button',{name:'Abrir El arte de prestar atención',exact:true}).click();
 await page.getByRole('button',{name:'Biblioteca',exact:true}).click();
 ...
 fs.writeFileSync('artifacts/browser-report.json',JSON.stringify({passed:true,checks:['real Markdown import','position survives reload','preferences survive reload','library filters/navigation','second import keeps first book','player dialog/Escape','chapter dialog/Escape','no overflow 1440/820/390/320','focus mode','zero browser errors',...(process.env.EVO_OFFLINE ? ['offline reload and reading','PDF worker cached'] : [])],errors},null,2));
 await browser.close();
